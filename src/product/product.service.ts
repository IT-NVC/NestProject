...
export class ProductService {
  ...
  async create(createProductDto: CreateProductDto): Promise<Product> {
    createProductDto.createDate = dateNow()
  ...
  async findOne(id: number): Promise<any> {
    let product = await this.productsRepository.query(
      `select * 
      from product,category
      where product.id_Category = category.id_category and product.id_Product = `+id+``
      );
    return product
  }
  ...
  async findOfUser (name):Promise<Product[]> {
    name = name.toLowerCase();
    let product;
    product = await this.productsRepository.query(
    `select *
    from product
    ...
    else
    {
      ...
      product = product.filter(function(product){
        return removeVietnameseTones(product.NameProduct).toLowerCase().includes(name);
      });
      return product
    }
  }
